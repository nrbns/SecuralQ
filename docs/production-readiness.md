...
|------|--------|----------------|
| Tenant isolation | **partial→near-done** | Core product rows use `org_id` + `tenant_visibility_sql` / fail-closed get: assets/vulns/risks/chats/engagements/RAG, agents/commands/campaigns, **scans, incidents, evidence, archives, gap remediations, intel watch, XDR events, compliance attestations/CMMC affirmations**, **evidence spine** (`control_runtime_state`, vault, requirements, canonical observations), **exceptions**, notification outbox stats. **Proven paths:** SSE push filter (`sse_push_allowed_for_client`), agent authenticate/list_commands/request_command fail-closed across orgs. **Intentional leftovers:** notifications stay recipient-scoped (personal inbox; `org_id` stamped only); lab `local` / `AUTH_ENABLED=false` still sees own/lab rows. Remaining proofs: object storage, AI search, worker fan-out. |
...
| Certificate rotation | **done (server)** | `app/agent_certs.py` issue/rotate/renew/revoke + `POST /api/agents/{id}/certificate/*`. Proxy mTLS on check-in + gateway when `AGENT_MTLS_PROXY_VERIFY`. Agent presents `agent.crt`/`agent.key`. Fleet CA/ACME still ops. |
| Signed commands | **partial→improved** | HMAC seal (`event_id` + `nonce` + `signature`) on delivery via `app/agent_security.py`. Optional Ed25519 via `AGENT_COMMAND_SIGNING_ALG`. **RT-17** mandatory seals when `AGENT_REQUIRE_COMMAND_SIGNATURE=true` (lab default off; production profile requires all five agent-security flags). |

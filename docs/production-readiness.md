...
|------|--------|----------------|
| Tenant isolation | **partial→near-done** | Core product rows use `org_id` + `tenant_visibility_sql` / fail-closed get: assets/vulns/risks/chats/engagements/RAG, agents/commands/campaigns, **scans, incidents, evidence, archives, gap remediations, intel watch, XDR events, compliance attestations/CMMC affirmations**, **evidence spine** (`control_runtime_state`, vault, requirements, canonical observations), **exceptions**, notification outbox stats. **Intentional leftovers:** notifications stay recipient-scoped (personal inbox; `org_id` stamped only); lab `local` / `AUTH_ENABLED=false` still sees own/lab rows. |
